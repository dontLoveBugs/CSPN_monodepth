...
class AffinityPropagate(nn.Module):
    ...
    def checkout(self, affinity, feature):
        ...
        assert affinity.dim() == feature.dim(), 'affinity matrix should have same number of dimension as feature,' \
                                                'but got affinity.dim()={} and feature.dim()={}'\
                                                .format(affinity.dim(),feature.dim())
        assert affinity.shape[0] == feature.shape[0], 'affinity matrix BatchSize should be same as feature,' \
        ...
        if self.op in ['Conv2d']:
            ...
            assert affinity.shape[1] == channels, 'affnity matrix should have {} channels,' \
                                                  'but got {}'.format(channels, affinity.shape[1])
            assert affinity.shape[2:] == feature.shape[2:], 'affinity matrix Height and Width should be same as feature,' \
        ...
        if self.op in ['Conv3d']:
            ...
            assert affinity.shape[1] == channels, 'affnity matrix should have {} channels,' \
                                                  'but got {}'.format(channels, affinity.shape[1])
            assert affinity.shape[2:] == feature.shape[2:], 'affinity matrix Depth, Height and Width should be same as feature,' \
                                                      'but got affinity D,H,W={},{},{} and feature D,H,W={},{},{}'\
                                                      .format(affinity.shape[2],affinity.shape[3],affinity.shape[4],
...
if __name__ == '__main__':
    ...
    if op == 'Conv2d':
        ...
        feature = torch.rand(B, inplanes, H, W).cuda()
        feature.requires_grad = True
    if op == 'Conv3d':
        # -------------------- 3D --------------------- #
        if isinstance(kernel_size, (list, tuple)):
            C = kernel_size[0]*kernel_size[1]*kernel_size[2]
        else:
            C = kernel_size**3
        B, D, H, W = batchsize, depth, height, width
        affinity = torch.rand(B, C, D, H, W).cuda()
        affinity.requires_grad = True
        feature = torch.rand(B, inplanes, D, H, W).cuda()
        feature.requires_grad = True

    # --------------------------------------------- #

    model = AffinityPropagate(op, iterations, kernel_size, stride, padding, dilation)
    model.cuda()
    loss = model(affinity, feature)
